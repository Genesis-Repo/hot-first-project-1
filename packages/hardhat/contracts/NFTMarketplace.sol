...
contract NFTMarketplace is ERC721Holder, Ownable {
    ...
    event SellerRated(address indexed seller, address indexed rater, uint256 rating, string review);
    event BuyerRated(address indexed buyer, address indexed rater, uint256 rating, string review);
    ...
    function buyNFT(address nftContract, uint256 tokenId, uint256 rating, string calldata review) external payable {
        ...
        sellerRatings[listing.seller].totalRatings++;
        sellerRatings[listing.seller].totalScore += rating;
    ...
    function rateSeller(address seller, uint256 rating, string calldata review) external {
        require(rating >= 1 && rating <= 5, "Rating must be between 1 and 5");

        sellerRatings[seller].totalRatings++;
        sellerRatings[seller].totalScore += rating;

        emit SellerRated(seller, msg.sender, rating, review);
    }

    // Function to rate a buyer
    function rateBuyer(address buyer, uint256 rating, string calldata review) external {
        require(rating >= 1 && rating <= 5, "Rating must be between 1 and 5");

        buyerRatings[buyer].totalRatings++;
        buyerRatings[buyer].totalScore += rating;

        emit BuyerRated(buyer, msg.sender, rating, review);
    }
